...
# Spring Boot SSL Tests Docker Image Build Script
# Builds a Spring Boot test container that runs SSL tests with wolfJSSE in FIPS mode
...
# Default values
IMAGE_NAME="spring-boot-wolfjsse"
TAG="latest"
VERBOSE=false
NO_CACHE=false
...
usage() {
    ...
    echo ""
    echo "Build the Spring Boot SSL tests Docker image with wolfJSSE FIPS support"
    echo ""
    echo "OPTIONS:"
    echo "  -n, --name NAME      Set image name (default: spring-boot-wolfjsse)"
    ...
    echo "Examples:"
    echo "  $0                                    # Basic build"
    echo "  $0 -n myspringboot -t v1.0           # Custom name and tag"
...
echo -e "${GREEN}=== Spring Boot SSL Tests Build (wolfJSSE FIPS) ===${NC}"
...
if docker build "${DOCKER_ARGS[@]}" -t "${IMAGE_NAME}:${TAG}" .; then
    ...
    echo ""
    echo -e "${BLUE}Run all SSL tests:${NC}"
    echo "  docker run --rm ${IMAGE_NAME}:${TAG}"
    echo ""
    echo -e "${BLUE}Test modules:${NC}"
    echo "  - spring-boot: Core SSL bundle tests"
    echo "  - spring-boot-autoconfigure: SSL auto-configuration tests"
    echo "  - spring-boot-actuator: Actuator SSL tests"
    echo ""
    echo -e "${BLUE}Run specific test class:${NC}"
    echo "  docker run --rm ${IMAGE_NAME}:${TAG} ./gradlew :spring-boot-project:spring-boot:test --tests 'SslBundleTest' --no-daemon"
    echo ""
    echo -e "${BLUE}Run specific module tests:${NC}"
    echo "  docker run --rm ${IMAGE_NAME}:${TAG} ./gradlew :spring-boot-project:spring-boot:test --tests '*Ssl*' --no-daemon"
    echo ""
