...
2. **`Dockerfile.provider-min`**: Uses `wolfProvider` to register wolfSSL as the default OpenSSL provider system-wide. Uses `python:3.12-alpine3.22` as the base image skipping the need to build Python.
3. **`Dockerfile.osp`**: Applies patches from [wolfSSL/osp](https://github.com/wolfSSL/osp/tree/master/Python) to completely replace OpenSSL in Python.
...
### 2. `Dockerfile.provider-min` (wolfProvider)
- **Method**: Registers wolfSSL as the default OpenSSL provider using `wolfProvider`. Uses `python:3.12-alpine3.22` as the base image.
- **Behavior**: wolfSSL is used for cryptographic operations, and OpenSSL remains available but is not used for cryptographic operations. All cryptographic operations are performed by wolfCrypt FIPS.
- **Python Version**: 3.12.11
- **Use Case**: Ideal for environments where OpenSSL is still needed alongside wolfSSL. Easiest method to update to new versions of the base image.

### 3. `Dockerfile.osp` (OpenSSL Replacement Patch)
...
- **Build**: Use `make build-provider`, `make build-provider-min`, or `make build-osp` to build images.
- **Run**: Use `make run-provider`, `make run-provider-min`, or `make run-osp` to test the containers.
...
- `Dockerfile.provider-min`: Builds the wolfProvider-based image. Uses `python:3.12-alpine3.22` as the base image.
